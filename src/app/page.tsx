...
const tiles = [
  ...
  { href: "/kcd", label: "질병분류표 (KCD)", desc: "코드·질병명 즉시 검색", icon: Wrench },
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
        {tiles.map(({ href, label, desc, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className="group rounded-2xl border bg-card p-5 transition hover:-translate-y-0.5 hover:border-primary hover:shadow-md md:p-6"
          >
            ...
            <div className="text-base font-semibold md:text-lg">{label}</div>
